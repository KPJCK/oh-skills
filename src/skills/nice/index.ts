...
const subcommands: { name: string; short: string; load: () => Promise<Cmd> }[] = [
  ...
  { name: "update-plan", short: "Append updates to an existing plan",       load: () => import("./commands/update-plan.ts") },
...
export async function run(args: string[]): Promise<void> {
  ...
  if (!sub) {
    ...
    emit("nice", [
      ...
      {
        type: "report",
        message: "Once the user picks an option, extract the leading word and run: bun ${CLAUDE_PLUGIN_ROOT}/src/cli.ts nice <subcommand>",
      },
    ]);
    return;
  }
  ...
  if (!found) {
    error(`unknown nice subcommand: ${sub}`, `expected one of: ${subcommands.map((s) => s.name).join(", ")}`);
    process.exit(2);
  }
